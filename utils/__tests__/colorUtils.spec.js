...
describe('parseGradient', () => {
    it('should replace colors with their valid forms', () => {
        expect(parseGradient('to top, red lighten-1, rgba(#000, .8)', colors, currentTheme)).toBe('to top, #ff6666, rgba(0,0,0, .8)');
        expect(parseGradient('to top, #fff, primary', colors, currentTheme)).toBe('to top, #fff, #1976d2');
        expect(parseGradient('to top, var(--foo), rgba(#0000, .6)', colors, currentTheme)).toBe('to top, var(--foo), rgba(0,0,0, .6)');
    });
});
