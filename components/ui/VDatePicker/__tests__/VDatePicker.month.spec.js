...
await wrapper.setProps({ value: '2005-11' })
    expect(wrapper.vm.tableDate).toBe('2005')
  })
...
await wrapper.setProps({
  ...
  it('should emit click/dblclick:month event', async () => {
    ...
    const wrapper = mountFunction({
      ...
      listeners: {
        'click:month': (value, event) => click(value, event instanceof Event),
        'dblclick:month': (value, event) => dblclick(value, event instanceof Event),
      },
    })
