...
describe('VDatePicker.js', () => { // eslint-disable-line max-statements
...
await wrapper.setProps({ value: '2005-11-11' })
    expect(wrapper.vm.tableDate).toBe('2005-11')
  })
...
await wrapper.setData({
  ...
  it('should emit click/dblclick:date event', async () => {
    ...
    const wrapper = mountFunction({
      ...
      listeners: {
        'click:date': (value, event) => click(value, event instanceof Event),
        'dblclick:date': (value, event) => dblclick(value, event instanceof Event),
      },
    })
