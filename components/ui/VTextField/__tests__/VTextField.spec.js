...
describe('VTextField.js', () => { // eslint-disable-line max-statements
    let mountFunction;
    beforeEach(() => {
    ...
    it('should not generate label', async () => {
        const wrapper = mountFunction();

        expect(wrapper.vm.genLabel()).toBeNull();

        await wrapper.setProps({ singleLine: true });
    ...
    it('should use counter value function', async () => {
        const wrapper = mountFunction({
            ...
            propsData: {
                counter: true,
                counterValue: value => (value || '').replace(/\s/g, '').length,
            },
        });
        ...
        await wrapper.setProps({
            counterValue: value => (value || '').replace(/ba/g, '').length,
        });
    ...
    it('should be single if using the filled prop with no label', async () => {
        const wrapper = mountFunction({
        ...
        await wrapper.setProps({ label: 'Foobar ' });
