<script>
/* eslint-disable no-eq-null,new-cap,no-unused-expressions */
...
import Mobile from '../../../mixins/mobile/index.js';
    ...
    computed: {
        ...
        computedMaxHeight() {
            ...
            const computedMaxHeight =
                this.$application.bottom +
                this.$application.footer +
                this.$application.bar;
            if (!this.clipped) {
                return computedMaxHeight;
            }

            return computedMaxHeight + this.$application.top;
        },
        computedTop() {
            if (!this.hasApp) {
                return 0;
            ...
            let computedTop = this.$application.bar;

            computedTop += this.clipped
                ? this.$application.top
                : 0;
...
<style lang="scss">
    ...
    .v-navigation-drawer--temporary {
        &:not(.v-navigation-drawer--close) {
            @include elevation($navigation-drawer-mobile-temporary-elevation);
        }
    }
</style>
