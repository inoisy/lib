...
await wrapper.setProps({ temporary: false })
    expect(wrapper.vm.$vuetify.application.left).toBe(256)
  })
...
await wrapper.setProps({ permanent: false })
    expect(wrapper.vm.$vuetify.application.left).toBe(0)
  })
...
await wrapper.setProps({ miniVariant: true })
    expect(wrapper.vm.$vuetify.application.left).toBe(56)

await wrapper.setProps({ miniVariant: false })
    expect(wrapper.vm.$vuetify.application.left).toBe(256)
  })
...
await wrapper.setProps({ app: false, temporary: false })
...
await wrapper.setProps({ app: true })
  ...
  it('should emit `update:mini-variant` when mouseenter/mouseleave', async () => {
    ...
    const wrapper = mountFunction({
      ...
      listeners: {
        'update:mini-variant': (value) => {
      await wrapper.setProps({ miniVariant: value })
          update(value)
        },
      },
    })
...
await wrapper.setProps({ expandOnHover: true })
    await wrapper.vm.$nextTick()

await wrapper.setProps({ expandOnHover: false })
    await wrapper.vm.$nextTick()
