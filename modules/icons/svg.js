...
export default class SVGManager extends Hookable {
    ...
    async unregisterSprite(sprite) {
        if (this.sprites[sprite] && this.sprites[sprite]) {
            ...
            try {
                await fs.unlink(join(this.output, sprite + '.svg'));
            } catch (e) {}
            // update json
            await this.writeJsonInfo();
        }
 ...
 */
    ...
    async writeSprite(sprite, directory) {
        ...
        await writeSVG(join(directory, `${sprite.name}.svg`), svg);
    }

    async optimizeSVG(name, content) {
        cleanupIDs.params.prefix = `${name}-`;
        const $data = await this.svgo.optimize(content, this.svgoConfig);
        return $data.data;
    }
}
