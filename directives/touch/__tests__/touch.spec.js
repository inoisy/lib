...
describe('touch.js', () => {
    let mountFunction;// : (value) => Wrapper;

    beforeEach(() => {
        mountFunction = (value = {}) => mount(Vue.component('Test', {
            directives: { Touch },
            render: h => h('div', {
                directives: [{
                    name: 'touch',
    ...
    it('should call directive handlers', async () => {
        const down = jest.fn();
        touch(mountFunction({ down })).start(0, 0)
            .end(0, 20);
        expect(down).toHaveBeenCalled();

        const up = jest.fn();
        touch(mountFunction({ up })).start(0, 0)
            .end(0, -20);
        expect(up).toHaveBeenCalled();

        const left = jest.fn();
        touch(mountFunction({ left })).start(0, 0)
            .end(-20, 0);
        expect(left).toHaveBeenCalled();

        const right = jest.fn();
        touch(mountFunction({ right })).start(0, 0)
            .end(20, 0);
    ...
    it('should call directive handlers if not straight down/up/right/left', async () => {
        const nope = jest.fn();
        const down = jest.fn();
        touch(mountFunction({ down, right: nope })).start(0, 0)
            .end(5, 20);
